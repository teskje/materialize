...
/// Test that each index in `BUILTINS_STATIC` appears immediately after the object it indexes,
/// modulo other indexes on the same object.
///
/// Ordering indexes as early as possible ensures that all objects depending on an indexed
/// relation can make use of the index.
#[mz_ore::test]
#[cfg_attr(miri, ignore)] // unsupported operation: can't call foreign function `rust_psm_stack_pointer` on OS `linux`
fn test_builtin_index_order() {
    ...
    let mut last_non_index_name: Option<String> = None;
    ...
    for builtin in BUILTINS_STATIC.iter() {
        match builtin {
            Builtin::Index(index) => {
                ...
                let on_name = match stmt {
                    Statement::CreateIndex(create_index) => create_index.on_name.to_string(),
                    _ => panic!("expected CreateIndex statement for {}", index.name),
                };
                let last_name = last_non_index_name.as_ref().unwrap_or_else(|| {
                    panic!("index {} appears before any non-index item", index.name)
                });

                assert_eq!(
                    &on_name, last_name,
                    "index {} not ordered immediately after {}",
                    index.name, on_name,
                );
            }
            other => {
                last_non_index_name = Some(format!("{}.{}", other.schema(), other.name()));
            }
        }
    }
}
